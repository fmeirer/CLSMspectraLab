...
% Update 2023-08-31: added 4th channel
...
for ii = 1:cl.nInput
    ...
    [Ich1,Ich2,Ich3,Ich4] = getIbinsProcessed(cl_bin,ii,'sum','c',{ch1,ch2,ch3,ch4},'x','y','c');
    ...
    % added 2023-08-31 ---
    Rch_4_123 = sum(Ich1,3)./Nf; Rch_4_123(isnan(Rch_4_123)) = [];
    figure; histogram(Rch_4_123,'Normalization',normalization); xlim([0 1]); caxis([0 1])
    xlabel('ch4/(ch1+ch2+ch3)'); ylabel(normalization)
    p.print([savenames{ii} '_bf' num2str(bin_factor) '_histogramPeak4_123'])
    ratio_mean(ii,6) = mean(Rch_4_123(:));
    ratio_std(ii,6) = std(Rch_4_123(:));
    % ---
    ...
    % added 2023-08-31 ---
    figure
    cl_bin.plotRatio_2D([],ii,1,1,ch4,[ch1 ch2 ch3],false); caxis([0 1])
    p.print([savenames{ii} '_bf' num2str(bin_factor) '_map4_123'])
    figure
    cl_bin.plotRatio_2D([],ii,1,1,ch4,[ch1 ch2 ch3],true); caxis([0 1])
    p.print([savenames{ii} '_bf' num2str(bin_factor) '_map4_123_norm'])
    % ----
...
if printFlag
    T = cell2table(num2cell(ratio_mean),'RowNames',savenames,'VariableNames',{'1/(1+2+3)','2/(1+2+3)','3/(1+2+3)','1/(1+3)','1+2+3','4/(1+2+3)'});
    writetable(T,fullfile(fp_save,'ratio_mean.csv'),'WriteRowNames',true)
    
    T = cell2table(num2cell(ratio_std),'RowNames',savenames,'VariableNames',{'1/(1+2+3)','2/(1+2+3)','3/(1+2+3)','1/(1+3)','1+2+3','4/(1+2+3)'});
